...
use App\Models\Article\ArticleTemplate as Template;
 ...
 * @link     接口文档链接
 */
class ArticleTemplate extends AdminController
{
    /**
     * 设置模型
     *
    ...
    public function delete($id)
    {
        ...
        $result = Article::query()->where('template_id', $id)
            ->get()
            ->isEmpty();
        if (!$result) {
            //当前要删除的模板关联到的文章设置为默认模板
            Article::query()->where('template_id',$id)->update(['template_id'=>1]);
        }
